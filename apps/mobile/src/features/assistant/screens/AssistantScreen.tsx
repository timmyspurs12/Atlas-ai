      ...
      >
        <View style={[styles.header, { borderColor: theme.colors.border }]}>
          <View style={styles.aiLogo}>
            <Sparkles size={21} color={palette.white} />
          </View>
        ...
        >
          <Card style={styles.privacyCard}>
            ...
            <AtlasText variant="caption" color={theme.colors.textMuted} style={styles.flex}>
              I only use locations people currently share with you. I cannot find anyone without
              permission.
            </AtlasText>
          </Card>
            ...
            >
              {message.role === 'assistant' ? (
                <View style={styles.botAvatar}>
                  <Bot size={17} color={palette.white} />
                </View>
              ) : null}
              <View
                style={[
                ...
                {message.action && message.action.type !== 'NONE' ? (
                  <Pressable
                    style={[
                      ...
                      {
                        borderColor:
                          message.role === 'user' ? 'rgba(255,255,255,0.25)' : theme.colors.border,
                      },
                    ]}
                  >
                    <AtlasText
                      variant="label"
                      color={message.role === 'user' ? palette.white : palette.blue}
                    >
                      {message.action.type === 'OPEN_PERSON'
                        ? 'Open live map'
                        : message.action.type === 'OPEN_TRIP'
                          ? 'Replay journey'
                          : 'View report'}
                    </AtlasText>
                    <ChevronRight
              ...
              <View style={styles.suggestionGrid}>
                {suggestions.map(({ text, icon: Icon, color }) => (
                  ...
                  >
                    ...
                    <AtlasText variant="caption" style={styles.flex}>
                      {text}
                    </AtlasText>
                  </Pressable>
                ))}
        ...
        >
          ...
          >
            ...
            <IconButton
              ...
              size={42}
              style={{ backgroundColor: input.trim() ? palette.blue : theme.colors.background }}
            />
          </View>
          <AtlasText variant="micro" color={theme.colors.textMuted} align="center">
            AI CAN MAKE MISTAKES · VERIFY TIME-SENSITIVE SAFETY INFO
          </AtlasText>
        </View>
      </KeyboardAvoidingView>
    </SafeAreaView>
...
function demoAnswer(question: string): AssistantResponse {
  ...
  if (normalized.includes('sarah') || normalized.includes('where')) {
    return {
      ...base,
      answer:
        'Sarah is moving near Victoria Island. Her location updated just now, and she chose to share precise location with you until 6:30 PM.',
      action: { type: 'OPEN_PERSON', userId: '77bf481a-2060-4e3b-96f0-d8fb50f70b73' },
      safetyNotice: null,
    };
  }
  if (normalized.includes('closest')) {
    return {
      ...base,
      answer:
        'Leo is closest at approximately 2.3 km away. Straight-line distance may differ from driving distance.',
      action: { type: 'OPEN_PERSON', userId: 'adbe9ec3-d817-48de-99fc-bfb078fe4dc3' },
      safetyNotice: 'Distance is approximate.',
    };
  }
  if (normalized.includes('replay') || normalized.includes('journey')) {
    return {
      ...base,
      answer:
        'I found yesterday’s main journey: 8.7 km over 34 minutes. It started in Ikoyi and ended on Victoria Island.',
      action: { type: 'OPEN_TRIP', tripId: '5ecfe467-ff7b-463f-9529-8e8c7df1b897' },
      safetyNotice: null,
    };
  }
  return {
    ...base,
    answer:
      'This week you travelled 42.8 km across 9 recorded trips — 12% more than last week. Saturday was your highest-travel day.',
