...
export function GeofencesScreen() {
  ...
  const toggle = (id: string, field: 'arrivals' | 'departures'): void => {
    setPlaces((current) =>
      current.map((place) => (place.id === id ? { ...place, [field]: !place[field] } : place)),
    );
  };
    ...
    <Screen contentStyle={styles.content}>
      <View style={styles.header}>
        ...
        <View style={styles.flex}>
          <AtlasText variant="h2">Places</AtlasText>
      ...
      <Card style={styles.consent}>
        ...
        <View style={styles.flex}>
          <AtlasText variant="label">Consent-aware alerts</AtlasText>
          <AtlasText variant="caption" color={theme.colors.textMuted}>
            Alerts for another person work only while they explicitly allow precise geofence access.
          </AtlasText>
        </View>
      </Card>
            ...
            <Card key={place.id} style={styles.placeCard}>
              <View style={styles.placeHeader}>
                <View style={styles.placeIcon}>
                  <Icon size={21} color={palette.blue} />
                </View>
                <View style={styles.flex}>
                  <AtlasText variant="h3">{place.name}</AtlasText>
                  <AtlasText variant="caption" color={theme.colors.textMuted}>
                    {place.radius} m radius · watching {place.subject}
                  </AtlasText>
                </View>
                <Pressable>
              ...
              <View
                style={[styles.miniMap, { backgroundColor: theme.dark ? '#101C2A' : '#E8EEF2' }]}
              >
                <View
                  style={[
                    styles.zoneOuter,
                    { borderColor: `${palette.blue}55`, backgroundColor: `${palette.blue}12` },
                  ]}
                >
                  <View style={styles.zonePin}>
                    <MapPin size={17} color={palette.white} />
                  </View>
                </View>
                <View
                  style={[styles.road, { backgroundColor: theme.dark ? '#334155' : palette.white }]}
                />
              </View>
              <View style={[styles.alertRow, { borderColor: theme.colors.border }]}>
                <View style={styles.flex}>
                  <AtlasText variant="label">Arrival alerts</AtlasText>
                  <AtlasText variant="caption" color={theme.colors.textMuted}>
                    Notify when entering the zone
                  </AtlasText>
                </View>
                <Switch
                  value={place.arrivals}
                  onValueChange={() => toggle(place.id, 'arrivals')}
                  trackColor={{ false: theme.colors.border, true: palette.teal }}
                />
              </View>
              <View style={styles.alertRow}>
                <View style={styles.flex}>
                  <AtlasText variant="label">Departure alerts</AtlasText>
                  <AtlasText variant="caption" color={theme.colors.textMuted}>
                    Notify when leaving the zone
                  </AtlasText>
                </View>
                <Switch
                  value={place.departures}
                  onValueChange={() => toggle(place.id, 'departures')}
                  trackColor={{ false: theme.colors.border, true: palette.teal }}
                />
              </View>
            </Card>
      ...
      >
        <View style={styles.backdrop}>
          ...
          <View style={[styles.modal, shadow, { backgroundColor: theme.colors.surface }]}>
            <View style={styles.header}>
              <View style={styles.flex}>
                <AtlasText variant="h2">Add a place</AtlasText>
                <AtlasText variant="caption" color={theme.colors.textMuted}>
                  Uses your current position as the centre.
                </AtlasText>
              </View>
              <IconButton icon={X} label="Close" onPress={() => setAdding(false)} size={40} />
            </View>
              ...
              {(Object.keys(iconFor) as Place['type'][]).map((item) => {
                ...
                return (
                  <Pressable
                    ...
                    style={[
                      styles.typeChoice,
                      { backgroundColor: type === item ? palette.blue : theme.colors.background },
                    ]}
                  >
                    <TypeIcon
                      size={18}
                      color={type === item ? palette.white : theme.colors.textMuted}
                    />
                  </Pressable>
                );
              })}
              ...
              {[100, 200, 500].map((value) => (
                <Pressable
                  ...
                  style={[
                    styles.radius,
                    {
                      backgroundColor: radius === value ? palette.blue : theme.colors.background,
                      borderColor: radius === value ? palette.blue : theme.colors.border,
                    },
                  ]}
                >
                  <AtlasText
                    variant="caption"
                    color={radius === value ? palette.white : theme.colors.text}
                  >
                    {value} m
                  </AtlasText>
                </Pressable>
